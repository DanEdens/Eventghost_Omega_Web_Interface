var omega={
  ...
  ShowSliderBox:function(o){//lable,command,preset,min,max,step,variable,callback,target,data,selectedObject
    ...
    if(top.conditionMode && top.recording){
      ...
      return top.showSliderBox(lable,callbackFunction,preData,minimum,maximum,step);
    }
  },
    ...
    if (showStates || ext=="init" || ext=="load" || ext=="reload"){
      ...
      for (var i=0; i<omega.subPages.length; i++){
        if (ext=="init" || ext=="load" || ext=="reload"){
          if (top.viewChoosen=="-" || top.files[omega.subPages[i]][10].indexOf(top.viewChoosen)!=-1){
              ...
              if (buttonMode=="rgb" && !top.recording && !top.dashEditing && !buttonAfterThis){
                ...
                if(buttonId1>=0){
                  if (top.conditionMode && top.recording){
                    ...
                    actionString=' onmouseup="top.showSceneConditionBox(\''+tempIds[0]+'/'+tempIds[1]+'\',\''+tempIds[2]+'/'+tempIds[3]+'\',\''+thisStateId2+'\');"';//#click#
                  }
                  else{
                    if (mouseWheel){
  ...
  buttonAction:function(buttonI,thisStateId,actionIndex,button,data){
    ...
    if(buttonOnlyRecord===0 || top.recording || top.dashEditing){
      ...
      if (buttonMode=="dim"){
        ...
        top.showNumberBox(tempText,omega.createButtonExecute(thisStateId2,buttonI,button,actionIndex),buttonModeArr[0]||"",buttonModeArr[1]||"0",buttonModeArr[2],buttonModeArr[3]);
      }
      else if (buttonMode=="sld"){
        top.sld.box.show(currentId,omega.createButtonExecute(thisStateId2,buttonI,button,actionIndex),buttonModeArr[0]||"",buttonModeArr[1]||"0",buttonModeArr[2],buttonModeArr[3],omega);
      }
