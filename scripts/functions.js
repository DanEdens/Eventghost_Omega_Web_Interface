...
function registerClient(){
  $.post("/empty", "ExecuteScript&eg.plugins.OMEGA.plugin.registerClient(self.getClientAddress())", function(data){
      thisIP=data;
    ...
    if (data[3]){
      ...
      if (parts["suffix"]==top.sysName+"buttons"){
        ...
        if (target[0]=="undefined"){
          ...
          tempText+=getText("files.json"+temp[0]+"/"+temp[1],temp[2]+"/"+temp[3]);
          if (dontReplaceBr){
            tempText+="<br>";
          }
          else{
...
function dataUpdate(ext){
  ...
  heartbeat=setTimeout("dataUpdate('std'); top.frames['chooseframe'].timeUpdate();",10000);
  var isReload = false;
  if (ext=="reload"){
  ...
  $.ajax({
    ...
    timeout: 0,
    cache: false,
    data: JSON.stringify({"method":"ExecuteScript","args":["eg.plugins.OMEGA.plugin.dataUpdate("+JSON.stringify(hashs)+","+JSON.stringify(uniHashs)+",'"+thisIP+"',"+isReload+")"]})
  })
  .fail(function(data) {
    ...
    if (data==false){
      ...
      for (var j=0; j<hashmodes.length; j++){
        ...
        if (hashData[mode]!=hashs[mode]){
          $.post("/empty",JSON.stringify({"method":"ExecuteScript","args":["eg.plugins.OMEGA.plugin.dataUpdate2('"+mode+"')"]}),dataUpdate2(mode));
...
function uniUnregister(varInBackend){
  ...
  $.ajax({
    ...
    url: "/empty",
    cache: false,
    data: "ExecuteScript&eg.plugins.OMEGA.plugin.uniUnregister('"+top.thisIP+"','"+varInBackend+"')"
  })
  .fail(function(data) {
...
function uniUpdate(id,varInBackend,targetFunctionName,targetFunction){
  ...
  $.ajax({
    ...
    url: "/empty",
    cache: false,
    data: "ExecuteScript&eg.plugins.OMEGA.plugin.uniUpdate('"+top.thisIP+"', '"+varInBackend+"')"
  })
  .fail(function(data) {
...
function sceneRemove(state,nr){
  if(state){
    ...
    var command="ExecuteScript&eg.plugins.OMEGA.plugin.sceneRemove("+nr+")";
    $.post("/empty",command);
  }
}
      ...
      if (frameID=="back"){
        ...
        if (openPageHistory.length>1){
          if (openPageHistory[0][0]=="fullscreen" || openPageHistory[1][0]=="fullscreen"){
            ...
            tempInitFrameID=filesIDArray[openPageHistory[0][2]];
          }
...
function dashAddBox(typ,data,lable,doIt,number,source){
  ...
  tempDataCommand=[data,0,typ,lable,false,false,number];
  if (!lable){
    if (typ==10){
...
function dashEditBox(i,callbackFunction){
  ...
  if (tempDataCommand[i][2]==0){
    ...
    content+='<tr><td colspan="2" class="w4">'+top.getText('showConfirmation',0)+':</td><td class="w2" align="center"><input id="confirmCheckbox" type="checkbox"></input></td></tr>';
...
function dashEditBoxOpen(i){
  ...
  if (tempDataCommand[i][2]==0){
    ...
    if (tempDataCommand[i][5]){
      document.getElementById("confirmCheckbox").checked=tempDataCommand[i][5];
    }
  }
...
function picPathChooserValuePrint(newVal){
  var command=JSON.stringify({"method":"ExecuteScript","args":["eg.plugins.OMEGA.plugin.getFilesForDropdown('dashboard')"]});
...
function dashEditBoxOpenOK(i){
  if (tempDataCommand[i][2]==0){
    ...
    tempDataCommand[i][5]=document.getElementById("confirmCheckbox").checked;
  }
...
var sld={
  box:{
    isOpen:false,
    tempDataCommand:false,
    show:function(currentId,callbackFunction,preData,minimum,maximum,step,sourcePageOmegaObject){
    ...
    onClose:function(currentId){
      ...
      sld.box.callbackFunction({OK:sld.box.tempDataCommand,data:sld.box.data});
    }
  },
...
function sceneActionsSave(nr,data){
  ...
  //var eventname=top.sysName+"sceneSaveActions&"+nr+"&"+encodeURIComponent(JSON.stringify(data));
  showBox(getText('sceneSaved',0));
  var command=JSON.stringify({'method':'ExecuteScript','args':['eg.plugins.OMEGA.plugin.sceneSaveActions('+nr+',"'+encodeURIComponent(JSON.stringify(data))+'")']});
  $.post("/empty",command, function(data){
...
function saveUserSettings(callbackFunction,wait){
  if (wait){
    ...
    userSettingsDeviceAll["default"]=userSettingsDevice;
    localStorage.userSettings=JSON.stringify(userSettingsDeviceAll);
    $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.saveUserSettings("'+encodeURIComponent('default')+'","'+encodeURIComponent(JSON.stringify(userSettings1))+'")', saveUserSettings2(callbackFunction));
  }
}
...
function Request(eventname,time,shadow,lable){
  ...
  if(time){
    if (lable){
      setTimeout('Request(\''+eventname+'\',false,'+shadow+',\''+lable+'\');',time);
    }
    else{
      setTimeout('Request(\''+eventname+'\',false,'+shadow+');',time);
    }
  }
  else{
      ...
      if (recording==false || forwardCommands){
        ...
        $.post("/empty",JSON.stringify(tevent))
          .always(function(){
            ...
            responseTimeout=setTimeout('$(responseShowerElement).hide();',50);
          });
      }
    }
