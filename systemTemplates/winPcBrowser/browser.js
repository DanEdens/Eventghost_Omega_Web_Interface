...
function bla(force) {
  ...
  if (advSearch==false && typeof myInstanceNr!="undefined"){
    ...
    $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.getBrowserValue("'+omega.deviceID+'",'+myInstanceNr+',4,1,'+refresh+')', function(data){
      if (data=="False"){
...
function browserStart(){
  if (top.States["devices"][omega.deviceID]["power"]==="[on]"){
    if (typeof top.browserInstanceNr[omega.pageID]=="undefined"){
      $.post('/empty','ExecuteScript&eg.plugins.OMEGA.plugin.startBrowserMenu("'+omega.deviceID+'",u"'+top.States["devices"][omega.deviceID]["browser"]["root"]+'", u"*", '+hideHiddenFiles+')', function(data){
        myInstanceNr=data;
  ...
  if (advSearch==false){
    if (target==-1){
      if (document.getElementById('textBoxTwo').value!=""){
        ...
        if (historie.length>1){
          isBack=1;
          $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.browserGoBack("'+omega.deviceID+'",'+myInstanceNr+')', function(data2){
            if (data2=="True"){
              historie.pop();
              bla();
            }
      ...
      isBack=0;
      $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.browserGoToParent("'+omega.deviceID+'",'+myInstanceNr+')', function(data2){
        if (data2=="True"){
      ...
      $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.browserExecute("'+omega.deviceID+'",20, u"", u"", '+myInstanceNr+', '+target+')', function(data2){
    ...
    if (target==-1 || target==-3){
      ...
      if (fileFolderArray[target]==2||fileFolderArray[target]==4){
        isBack=0;
        loadTimer=setTimeout("loading();",500);
        $.post('/empty', JSON.stringify({"method":"ExecuteScript","args":['eg.plugins.OMEGA.plugin.browserOpenLocation("'+omega.deviceID+'",'+myInstanceNr+',u"'+encodeURIComponent(rowsArray[target*2+1])+'")']}), function(data2){
          clearTimeout(loadTimer);
          if (data2){
            advSearch=false;
...
function openLocation(path){
  loadTimer=setTimeout("loading();",500);
  $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.browserOpenLocation("'+omega.deviceID+'",'+myInstanceNr+',u"'+encodeURIComponent(path)+'")', function(data2){
    clearTimeout(loadTimer);
    if (data2=="True"){
      advSearch=false;
      searchTargetLength=-1;
...
function deleteCancel(){
  if (deleteOkV==false){
    ...
    if (advSearch){
      ...
      $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.getBrowserValues("'+omega.deviceID+'",'+myInstanceNr+',12,\''+JSON.stringify(tempTargetIds)+'\')', function(data2){
...
function openWithF(targets){
  ...
  if (advSearch){
    ...
    $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.getBrowserValues("'+omega.deviceID+'",'+myInstanceNr+',1,\''+JSON.stringify(tempTargetIds)+'\')', function(data2){
...
function propertiesF(targets){
  ...
  $.post('/empty', JSON.stringify({'method':'ExecuteScript','args':['eg.plugins.OMEGA.plugin.getProperties(\''+omega.deviceID+'\','+myInstanceNr+', \''+JSON.stringify(target2).replace(/\\/g, "\\\\")+'\', '+mode+')']}), function(data){
...
function searchRecus(){
  ...
  if (searchStr!=""){
    ...
    $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.serachSubFolders("'+omega.deviceID+'",'+myInstanceNr+',"'+escape(searchStr)+'")', function(data){
      if (data=="True"){
        loading();
      }
      else{
...
omega.OnUpdate(function(ext){
  if (ext=="data" && typeof myInstanceNr!="undefined"){
    if (setToLoading && top.States["devices"][omega.deviceID]["browser"][myInstanceNr]["newSearchData"]==1){
      $.post('/empty', 'ExecuteScript&eg.plugins.OMEGA.plugin.getBrowerSearchResult("'+omega.deviceID+'",'+myInstanceNr+')', function(data){
        if (data=="False"){
